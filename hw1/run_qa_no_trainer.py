...
def parse_args():
    ...
    parser.add_argument("--do_predict", default=0, action="store_true", help="To do prediction on the question answering model")
    parser.add_argument(
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        with open(args.dataset_name, 'r') as context:
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            answers["answer_start"].append(answers["start"])
            tmp = answers["text"]
            answers["text"] = []
            answers["text"].append(tmp)
            print(answers)
            '''
    ...
    def prepare_validation_features(examples):
        ...
        examples[context_column_name] =[
            context_json[ind] for ind in examples[context_column_name]
        ]
        '''
        #print(examples[context_column_name])
        # Tokenize our examples with truncation and maybe padding, but keep the overflows using a stride. This results
    ...
    if args.validation_file != None:
        ...
        for i in range(len(eval_examples)):
            answers = eval_examples[i]["answer"]
            answers["answer_start"] = []
            answers["answer_start"].append(answers["start"])
            tmp = answers["text"]
            answers["text"] = []
            answers["text"].append(tmp)
            eval_examples[i]["answer"]=answers
            print(eval_examples[i])
            eval_examples[i]["answer"]["answer_start"] = []
            eval_examples[i]["answer"]["answer_start"].append(eval_examples[i]["answer"]["start"])
            tmp = eval_examples[i]["answer"]["text"]
            eval_examples[i]["answer"]["text"] = []
            eval_examples[i]["answer"]["text"].append(tmp)
        '''
    ...
    if args.do_predict:
        predict_dataloader = accelerator.prepare(
            predict_dataloader
        )
    device = accelerator.device
    model.to(device)
    # Train!
    if args.do_predict == 0:
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            tokenizer.save_pretrained(args.output_dir, ensure_ascii=False)
            if args.push_to_hub:
                repo.push_to_hub(commit_message="End of training", auto_lfs_prune=True,ensure_ascii=False)
            if args.do_predict == 0:
                logger.info(json.dumps(eval_metric, indent=2, ensure_ascii=False))
